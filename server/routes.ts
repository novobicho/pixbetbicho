...
import { createBetValidator } from "./validators/bet.validators";
import { createWithdrawalValidator } from "./validators/payment.validators";
import { drawScheduler } from "./services/draw-scheduler";
import { autoUpdateService } from "./services/auto-update-results";
...
import { resultScraper } from "./services/result-scraper";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/health', (req, res) => {
    res.status(200).json({
      ...
      webhookUrl: process.env.WEBHOOK_URL || 'NOT_CONFIGURED',
      server: {
  ...
  // Rota para forçar atualização de todos os resultados pendentes
  app.post('/api/admin/draws/update-all', requireAdmin, async (req, res) => {
    try {
      console.log('Forçando atualização de todos os resultados pendentes...');
      await autoUpdateService.updatePendingDraws();
      res.json({ success: true, message: 'Atualização iniciada com sucesso' });
    } catch (error) {
      console.error('Erro ao atualizar resultados:', error);
      res.status(500).json({ success: false, message: 'Erro ao atualizar resultados' });
    }
  });
  ...
  // ========== Rotas para Templates de Sorteio (Recorrentes) ==========

  // Listar templates
  app.get("/api/draw-templates", requireAuth, async (req, res) => {
  ...
  // Criar template
  app.post("/api/draw-templates", requireAuth, async (req, res) => {
    if (!req.user?.isAdmin) return res.sendStatus(403);
    ...
    try {
      ...
      // Opcional: Acionar o scheduler para criar sorteios imediatamente se aplicável
  ...
  // Atualizar template
  app.put("/api/draw-templates/:id", requireAuth, async (req, res) => {
    if (!req.user?.isAdmin) return res.sendStatus(403);
  ...
  // Deletar template
  app.delete("/api/draw-templates/:id", requireAuth, async (req, res) => {
    if (!req.user?.isAdmin) return res.sendStatus(403);
  ...
  // Get completed draws (results) - Public
  app.get("/api/draws/results", async (req, res) => {
    try {
  ...
  // Get recent draws (results) - Public
  ...
  app.put("/api/draws/:id", requireAdmin, async (req, res) => {
    try {
      ...
      // Verificar se estão sendo adicionados resultados
      const hasResults = drawData.resultAnimalId ||
      ...
      if (hasResults && drawData.resultAnimalId) {
        // Se tem resultados, usar updateDrawResult para processar apostas
        console.log("🎯 Detectado adição de resultados - processando apostas...");
        ...
        // Atualização normal (sem resultados)
  ...
  app.put("/api/draws/:id/result", requireAdmin, async (req, res) => {
    try {
      ...
      const {
        ...
        animalId6, // 6º prêmio (opcional)
        animalId7, // 7º prêmio (opcional)
        animalId8, // 8º prêmio (opcional)
        animalId9, // 9º prêmio (opcional)
        animalId10, // 10º prêmio (opcional)
        ...
        resultNumber6, // Número do 6º prêmio (opcional)
        resultNumber7, // Número do 7º prêmio (opcional)
        resultNumber8, // Número do 8º prêmio (opcional)
        resultNumber9, // Número do 9º prêmio (opcional)
        resultNumber10 // Número do 10º prêmio (opcional)
      } = req.body;
      ...
      console.log(`Processing draw result: Draw ID: ${drawId}
        ...
        6º prêmio: Animal ${animalId6 || 'não definido'}, Número ${resultNumber6 || 'não definido'}
        7º prêmio: Animal ${animalId7 || 'não definido'}, Número ${resultNumber7 || 'não definido'}
        8º prêmio: Animal ${animalId8 || 'não definido'}, Número ${resultNumber8 || 'não definido'}
        9º prêmio: Animal ${animalId9 || 'não definido'}, Número ${resultNumber9 || 'não definido'}
        10º prêmio: Animal ${animalId10 || 'não definido'}, Número ${resultNumber10 || 'não definido'}
      `);
      ...
      const formattedNumber6 = resultNumber6 ? resultNumber6.padStart(4, '0') : undefined;
      const formattedNumber7 = resultNumber7 ? resultNumber7.padStart(4, '0') : undefined;
      const formattedNumber8 = resultNumber8 ? resultNumber8.padStart(4, '0') : undefined;
      const formattedNumber9 = resultNumber9 ? resultNumber9.padStart(4, '0') : undefined;
      const formattedNumber10 = resultNumber10 ? resultNumber10.padStart(4, '0') : undefined;
  ...
  // Atualizar resultado automaticamente (buscar do site)
  app.post("/api/draws/:id/auto-update", requireAdmin, async (req, res) => {
    try {
      const drawId = Number(req.params.id);

      logger.info(`[API] Solicitação de atualização automática para extração ID: ${drawId}`);

      const { autoUpdateService } = await import("./services/auto-update-results");
      const success = await autoUpdateService.updateDrawById(drawId);

      if (!success) {
        return res.status(404).json({
          message: "Não foi possível atualizar o resultado. Verifique se a extração existe e se o nome corresponde ao site."
        });
      }

      // Buscar extração atualizada
      ...
      logger.info(`[API] Extração ${drawId} atualizada automaticamente com sucesso`);
      res.json(updatedDraw[0]);
    } catch (error) {
      logger.error(`[API] Erro ao atualizar resultado automaticamente:`, error);
      res.status(500).json({ message: "Erro ao buscar resultado automaticamente" });
    }
  });

  // Atualizar todos os resultados automaticamente (buscar do site)
  app.post("/api/draws/auto-update-all", requireAdmin, async (req, res) => {
    try {
      logger.info(`[API] ========== ATUALIZAR TODOS OS SORTEIOS ==========`);
      logger.info(`[API] Solicitação de atualização automática para TODOS os sorteios pendentes do dia`);
      logger.info(`[API] Usuário: ${req.user ? req.user.username : 'não autenticado'}`);

      const { autoUpdateService } = await import("./services/auto-update-results");

      // Buscar todos os sorteios pendentes de hoje
      const today = new Date();
      ...
      logger.info(`[API] Encontrados ${pendingDraws.length} sorteios pendentes para atualizar`);

      let successCount = 0;
      let failCount = 0;

      // Atualizar cada sorteio
      for (const draw of pendingDraws) {
        try {
          const success = await autoUpdateService.updateDrawById(draw.id);
          if (success) {
            successCount++;
            logger.info(`[API] ✅ Sorteio ${draw.name} atualizado com sucesso`);
          } else {
            failCount++;
            logger.warn(`[API] ⚠️ Não foi possível atualizar o sorteio ${draw.name}`);
          }
        } catch (error) {
          failCount++;
          logger.error(`[API] ❌ Erro ao atualizar sorteio ${draw.name}:`, error);
        }
      }

      logger.info(`[API] Atualização em lote concluída: ${successCount} sucessos, ${failCount} falhas`);
      logger.info(`[API] ========== FIM ATUALIZAR TODOS ==========`);
      ...
      logger.error(`[API] Erro ao atualizar resultados em lote:`, error);
      res.status(500).json({ message: "Erro ao buscar resultados automaticamente" });
    }
  });

  // Limpar resultados do dia anterior manualmente (admin only)
  app.post("/api/draws/clear-previous-day", requireAdmin, async (req, res) => {
    try {
      logger.info(`[API] Solicitação manual de limpeza de resultados do dia anterior`);

      const { schedulerService } = await import("./services/scheduler");
      const result = await schedulerService.clearPreviousDayResults();

      if (result.success) {
        logger.info(`[API] Limpeza manual concluída: ${result.cleaned} sorteios limpos`);
        res.json({
        ...
        res.status(500).json({
          message: "Erro ao limpar resultados",
          error: result.error
        });
      }
    } catch (error) {
      logger.error(`[API] Erro ao limpar resultados do dia anterior:`, error);
      res.status(500).json({ message: "Erro ao limpar resultados" });
    }
  });


  // Reprocessar apostas de uma extração (admin only)
  app.post("/api/draws/:id/reprocess-bets", requireAdmin, async (req, res) => {
    try {
      const drawId = Number(req.params.id);

      console.log(`[REPROCESS] Iniciando reprocessamento de apostas para extração ID: ${drawId}`);
      ...
      if (draw.status !== "completed") {
        return res.status(400).json({ message: "Extração ainda não tem resultados" });
      }

      if (!draw.resultAnimalId) {
        return res.status(400).json({ message: "Extração não tem 1º prêmio definido" });
      }

      console.log(`[REPROCESS] Extração encontrada: ${draw.name}, Status: ${draw.status}`);
      console.log(`[REPROCESS] Chamando updateDrawResult para reprocessar...`);

      // Chamar updateDrawResult para reprocessar as apostas
      await storage.updateDrawResult(
        draw.id,
        draw.resultAnimalId,
        draw.resultAnimalId2 || undefined,
        draw.resultAnimalId3 || undefined,
        draw.resultAnimalId4 || undefined,
        draw.resultAnimalId5 || undefined,
        draw.resultAnimalId6 || undefined,
        draw.resultAnimalId7 || undefined,
        draw.resultAnimalId8 || undefined,
        draw.resultAnimalId9 || undefined,
        draw.resultAnimalId10 || undefined,
        draw.resultNumber1 || undefined,
        draw.resultNumber2 || undefined,
        draw.resultNumber3 || undefined,
        draw.resultNumber4 || undefined,
        draw.resultNumber5 || undefined,
        draw.resultNumber6 || undefined,
        draw.resultNumber7 || undefined,
        draw.resultNumber8 || undefined,
        draw.resultNumber9 || undefined,
        draw.resultNumber10 || undefined
      );
  ...
  // Create bet
  app.post("/api/bets", requireAuth, validate(createBetValidator), async (req, res) => {
    try {
      const userId = req.user!.id;
      logger.info(`Creating bet for user ID: ${userId}`);
      logger.debug("Bet request data:", req.body);
      logger.debug(`DEBUG - Bet request useBonusBalance: ${req.body.useBonusBalance} (${typeof req.body.useBonusBalance})`);
      ...
      // Validação de horário do sorteio (Lógica corrigida para UTC-3 e regra de 10 minutos)
      const now = new Date();
      ...
      // Comparar: O sorteio deve ser no futuro (maior que o tempo de corte)
      // drawDateTime está em "UTC que representa BRT"
      // cutoffTime também está em "UTC que representa BRT"
      if (drawDateTime <= cutoffTime) {
        ...
        return res.status(400).json({
          message: "Este sorteio já começou ou está no período de fechamento (10 min antes)",
          detail: "Apostas só são permitidas até 10 minutos antes do sorteio."
        });
      }
  ...
  app.post("/api/admin/upload-image", requireAdmin, async (req, res) => {
    try {
      ...
      if (imageType === 'logo' || imageType === 'bannerDesktop' || imageType === 'bannerMobile' || imageType === 'bannerDashboard') {
        ...
        pathsToSave.push(path.join(process.cwd(), 'client', 'public', fileName)); // Raiz
        pathsToSave.push(path.join(process.cwd(), 'client', 'public', 'img', fileName)); // Pasta img
      }

      // Se estiver em produção, salvar também no diretório dist/public que é o que está sendo servido
      if (process.env.NODE_ENV === 'production') {
        if (imageType === 'logo' || imageType === 'bannerDesktop' || imageType === 'bannerMobile') {
          pathsToSave.push(path.join(process.cwd(), 'dist', 'public', 'img', fileName));
        } else {
          pathsToSave.push(path.join(process.cwd(), 'dist', 'public', fileName));
          pathsToSave.push(path.join(process.cwd(), 'dist', 'public', 'img', fileName));
        }
      }
      ...
      // Salvar em todos os caminhos identificados
      for (const savePath of pathsToSave) {
        try {
          ...
          // Não interromper se falhar em um local, tentar nos outros
        }
      }
      ...
      for (const transaction of pendingTransactions) {
        try {
          ...
          // --- LOGICA CODEX PAY ---
            ...
            try {
              console.log(`🔍 CODEXPAY: Verificando status na API para transação ${transaction.id} (ExternalId: ${transaction.externalId})`);
              const paymentData = await codexpayService.getPaymentStatus(transaction.externalId);

              // Extrair status de forma resiliente
              const data = paymentData.transaction || paymentData.data || paymentData;
              const status = String(data.status || paymentData.status || "").toUpperCase();

              console.log(`ℹ️ CODEXPAY: Status retornado: ${status}`);

              if (['PAID', 'COMPLETED', 'COMPLETO', 'APPROVED', 'SUCESSO', 'CONCLUIDO', 'PAGO'].includes(status)) {
                console.log(`✅ CODEXPAY: Confirmando ${transaction.type === 'withdrawal' ? 'SAQUE' : 'PAGAMENTO'} para usuário ${transaction.userId}, valor: ${transaction.amount}`);

                // 1. Atualizar status da transação de pagamento no banco
                await storage.updateTransactionStatus(transaction.id, "completed", transaction.externalId, transaction.externalUrl || undefined, paymentData);

                if (transaction.type === 'withdrawal') {
                  // Lógica para SAQUE
                  const responseData = transaction.gatewayResponse as any;
                  const internalWithdrawalId = responseData?.internalWithdrawalId || responseData?.withdrawalId;

                  if (internalWithdrawalId) {
                    await storage.updateWithdrawalStatus(
                      internalWithdrawalId,
                      'approved',
                      undefined,
                      undefined,
                      'Saque confirmado via Polling da API CodexPay'
                    );
                    console.log(`✅ CODEXPAY: Status do saque ID ${internalWithdrawalId} atualizado para aprovado via Polling.`);
                  }
                } else {
                  // Lógica para DEPÓSITO
                  // 2. Processar bônus de primeiro depósito
                  try {
                    const systemSettings = await storage.getSystemSettings();
                    if (systemSettings?.firstDepositBonusEnabled) {
                      const hasBonus = await storage.hasUserReceivedFirstDepositBonus(transaction.userId);
                      if (!hasBonus) {
                        ...
                        if (bonusAmount > 0) {
                          ...
                          await storage.updateUserBonusBalance(transaction.userId, bonusAmount);
                        }
                      }
                    }
                  } catch (bonusErr) {
                    console.error("❌ CODEXPAY: Erro ao processar bônus no check-pending:", bonusErr);
                  }

                  // 3. Atualizar saldo principal (apenas para depósito!)
                  await storage.updateUserBalance(transaction.userId, Number(transaction.amount));

                  // 4. Criar registro de transação financeira
                  await storage.createTransaction({
                    userId: transaction.userId,
                    type: "deposit",
                    amount: Number(transaction.amount),
                    description: "Depósito via CodexPay (Confirmado via Polling)",
                    relatedId: transaction.id,
                  });
                }

                updatedCount++;
                results.push({ transactionId: transaction.id, status: "completed", message: `Confirmado via CodexPay API (${transaction.type})` });
                verified = true;
              }
            } catch (err) {
  ...
  app.post('/api/withdrawals', requireAuth, validate(createWithdrawalValidator), async (req, res) => {
    try {
      const userId = req.user.id;
      const { amount, pixKey: reqPixKey, pixKeyType: reqPixKeyType } = req.body;

      // Buscar usuário atualizado do banco (saldo em tempo real)
      ...
      if (freshUser.balance < withdrawalAmount) {
        return res.status(400).json({
          message: `Saldo insuficiente. Disponível: R$ ${freshUser.balance.toFixed(2)}`
        });
      }

      // Verificar se saques estão permitidos
      const sysSettings = await storage.getSystemSettings();
      if (sysSettings && !sysSettings.allowWithdrawals) {
        return res.status(403).json({ message: "Saques estão temporariamente desativados" });
      }

      // Usar chave PIX do request ou CPF do usuário como fallback
      const pixKey = reqPixKey || freshUser.defaultPixKey || freshUser.cpf;
      const pixKeyType = reqPixKeyType || 'cpf';

      if (!pixKey) {
        return res.status(400).json({
          message: "Chave PIX não configurada. Adicione seu CPF ou chave PIX nas configurações da conta."
        });
      }
      ...
      if (codexpayGateway?.isActive) {
        // === PROCESSAMENTO VIA CODEXPAY ===
        try {
          const codexpayService = await createCodexPayService();
          const ourExternalId = `withdrawal_${userId}_${Date.now()}`;

          const webhookUrl = process.env.WEBHOOK_URL
            ? `${process.env.WEBHOOK_URL}/api/codexpay/webhook`
            : `https://${process.env.REPL_SLUG || 'app'}.repl.co/api/codexpay/webhook`;

          const keyType = (pixKeyType === 'random' ? 'RANDOM' : pixKeyType.toUpperCase()) as 'CPF' | 'EMAIL' | 'PHONE' | 'RANDOM';

          console.log(`💎 CODEXPAY: Chamando API de saque para usuário ${userId}...`);

          const gatewayWithdrawal = await codexpayService.createWithdrawal({
            amount: withdrawalAmount,
            name: freshUser.name || freshUser.username,
            document: freshUser.cpf || '00000000000',
            externalId: ourExternalId,
            pixKey,
            pixKeyType: keyType,
            description: `Saque plataforma - usuário ${userId}`,
            callbackUrl: webhookUrl
          });

          // Criar registro interno de saque
          const [internalWithdrawal] = await pool.query(`
            INSERT INTO withdrawals (user_id, amount, pix_key, pix_key_type, status, requested_at, notes)
          ...
          `, [
            ...
            withdrawalAmount,
            pixKey,
            pixKeyType.toLowerCase(),
            `Processando via CodexPay. External ID: ${ourExternalId}`
          ]);

          const withdrawalRecord = internalWithdrawal.rows[0];

          // Criar transação de pagamento para rastreamento
          await storage.createPaymentTransaction({
            ...
            status: 'pending',
            type: 'withdrawal',
            externalId: gatewayWithdrawal.id || ourExternalId,
            gatewayResponse: {
              internalWithdrawalId: withdrawalRecord.id,
              ourExternalId,
              gatewayStatus: gatewayWithdrawal.status
            }
          });

          // Debitar saldo do usuário imediatamente
          await storage.updateUserBalance(userId, -withdrawalAmount);
          ...
          await storage.createTransaction({
            ...
            type: 'withdrawal',
            amount: withdrawalAmount,
            description: `Saque PIX via CodexPay - ${pixKey}`,
            relatedId: withdrawalRecord.id
          });

          console.log(`✅ CODEXPAY: Saque de R$ ${withdrawalAmount} iniciado para usuário ${userId}. Status: ${gatewayWithdrawal.status}`);
          ...
          return res.status(201).json({
            ...
            notes: `Processando via CodexPay`
          });

        } catch (codexpayError) {
          console.error(`❌ CODEXPAY: Erro ao processar saque via API:`, codexpayError);
          // Se a API falhar, criar saque pendente para aprovação manual
          console.log(`⚠️ Criando saque pendente para aprovação manual (fallback)...`);
        }
      }

      // === FALLBACK: Saque manual / sem gateway CodexPay ===
      ...
      console.log(`📋 Saque ID=${withdrawal.id} criado com status: ${withdrawal.status}`);
  ...
  async function checkPushinPayBalance(): Promise<number> {
    try {
      ...
      console.error("Erro ao verificar saldo PushinPay:", error);
      return 0;
    }
  }

  async function checkCodexPayBalance(): Promise<number> {
    try {
      const codexpayService = await createCodexPayService();
      return await codexpayService.getBalance();
    } catch (error) {
      console.error("Erro ao verificar saldo CodexPay:", error);
      return 0;
    }
  }
        ...
        if (codexpayGateway?.isActive) {
          try {
            ...
            await storage.createPaymentTransaction({
              ...
              gatewayResponse: { internalWithdrawalId: processingWithdrawal.id }
            });
  ...
  app.post("/api/codexpay/create-pix-payment", requireAuth, async (req, res) => {
    try {
      ...
      // Validar valor mínimo (R$ 10,00 conforme docs)
      if (!amount || Number(amount) < 10) {
        return res.status(400).json({
          success: false,
          error: 'O valor mínimo para depósito é R$10,00'
        });
      }
      ...
      // Gerar ID externo único (será o externalId que enviamos para o gateway)
      const ourExternalId = `deposit_${user.id}_${Date.now()}`;
      ...
      const payment = await codexpayService.createPixPayment({
        ...
        customerEmail: user.email || `${user.username}@pixbet.app`,
        customerDocument: user.cpf || '00000000000',
        callbackUrl: webhookUrl
      });

      // Guardar no banco: externalId = transactionId do gateway (para o webhook achar)
      const gateway = await storage.getPaymentGatewayByType("codexpay");
      const dbTransaction = await storage.createPaymentTransaction({
        userId: user.id,
        amount: Number(amount),
        gatewayId: gateway?.id || 0,
        status: 'pending',
        type: 'deposit',
        // Guardar o transactionId do gateway como externalId principal
        externalId: payment.transactionId || payment.id,
        // E guardar nosso ID externo na resposta para referência
        gatewayResponse: {
          ourExternalId,
          gatewayTransactionId: payment.transactionId || payment.id,
          qrCode: payment.qrCode,
          status: payment.status,
          amount: payment.amount,
          useBonus: useBonus || false
        }
      });
      ...
      // Resposta compatível com o frontend (deposit-dialog.tsx)
      // O frontend espera: transactionId, payment.qrCode, pixCopyPasteCode, amount
      res.json({
        success: true,
        transactionId: dbTransaction.id,  // ID interno do banco (para polling)
        amount: Number(amount),
        payment: {
          id: payment.transactionId || payment.id,
          transactionId: dbTransaction.id,
          amount: payment.amount || Number(amount),
          // qrCode pode ser base64 ou string EMV
          qrCode: payment.qrCode,
          qrCodeImage: payment.qrCode,    // alias para compatibilidade
          status: payment.status
        },
        // Campos diretos para compatibilidade com o frontend
        pixCopyPasteCode: payment.qrCode,   // Código PIX copia e cola
        qrCodeUrl: null                      // Não disponível na CodexPay
      });
    } catch (error) {
      ...
      res.status(500).json({
        success: false,
        error: 'Erro ao criar pagamento via CodexPay',
        details: errMsg
      });
    }
  });

  // Criar saque PIX com CODEXPAY (rota direta)
  app.post("/api/codexpay/create-pix-withdrawal", requireAuth, async (req, res) => {
    try {
      ...
      if (!pixKey) {
        return res.status(400).json({ success: false, error: 'Chave PIX obrigatória' });
      }
      ...
      // Verificar saldo atual do usuário no banco
      const freshUser = await storage.getUser(user.id);
      if (!freshUser || freshUser.balance < Number(amount)) {
        return res.status(400).json({
          success: false,
          error: `Saldo insuficiente. Disponível: R$ ${(freshUser?.balance || 0).toFixed(2)}`
        });
      }

      const codexpayService = await createCodexPayService();
      const ourExternalId = `withdrawal_${user.id}_${Date.now()}`;
      ...
      const keyType = (pixKeyType || 'CPF').toUpperCase() as 'CPF' | 'EMAIL' | 'PHONE' | 'RANDOM';
      ...
      const withdrawal = await codexpayService.createWithdrawal({
        ...
        pixKey,
        pixKeyType: keyType,
        description: `Saque plataforma - usuário ${user.id}`,
        callbackUrl: webhookUrl
      });

      // 1. Registrar saque interno
      const internalWithdrawal = await storage.createWithdrawal({
        userId: user.id,
        amount: Number(amount),
        pixKey,
        pixKeyType: keyType.toLowerCase()
      });

      // 2. Criar transação de pagamento para rastreamento
      const gateway = await storage.getPaymentGatewayByType("codexpay");
      await storage.createPaymentTransaction({
        userId: user.id,
        amount: Number(amount),
        ...
        status: 'pending',
        type: 'withdrawal',
        externalId: withdrawal.id || ourExternalId,
        gatewayResponse: {
          internalWithdrawalId: internalWithdrawal.id,
          ourExternalId,
          gatewayStatus: withdrawal.status
        }
      });

      // 3. Debitar saldo do usuário imediatamente
      await storage.updateUserBalance(user.id, -Number(amount));
      ...
      await storage.createTransaction({
        ...
        type: 'withdrawal',
        amount: Number(amount),
        description: `Saque PIX via CodexPay - Chave: ${pixKey}`,
        relatedId: internalWithdrawal.id
      });

      console.log('✅ CODEXPAY: Saque PIX iniciado com sucesso:', withdrawal.id);

      res.json({
        success: true,
        withdrawal: {
          id: withdrawal.id,
          amount: withdrawal.amount || Number(amount),
          status: withdrawal.status
        }
      });
    } catch (error) {
      ...
      res.status(500).json({
        success: false,
        error: 'Erro ao criar saque via CodexPay',
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });
  ...
  app.post("/api/codexpay/webhook", async (req, res) => {
    try {
      ...
      // Responder IMEDIATAMENTE com 200 para o gateway não retentar
      // O processamento real acontece a seguir
      res.status(200).json({ success: true, message: 'Webhook received' });

      // Extrair dados de forma resiliente (estrutura pode variar)
      const data = payload.transaction || payload.data || payload;

      // O transactionId é o ID que o gateway nos deu (retornado no createPixPayment/createWithdrawal)
      const gatewayTransactionId =
        data.transaction_id || data.transactionId || data.id ||
        payload.transaction_id || payload.transactionId || payload.id;

      // O externalId é o nosso ID que enviamos ao criar o pagamento (deposit_userId_timestamp)
      const ourExternalId =
        data.external_id || data.externalId || data.reference ||
        payload.external_id || payload.externalId || payload.reference;

      console.log('🔍 CODEXPAY WEBHOOK: IDs recebidos:', { gatewayTransactionId, ourExternalId });

      // Buscar transação - primeiro pelo transactionId do gateway (que guardamos como externalId)
      let transaction;

      if (gatewayTransactionId) {
        transaction = await storage.getPaymentTransactionByGatewayId(String(gatewayTransactionId));
        if (transaction) {
          console.log(`✅ CODEXPAY WEBHOOK: Transação encontrada pelo gatewayTransactionId: ${gatewayTransactionId}`);
        }
      }

      // Se não achou, tentar pelo nosso external_id
      if (!transaction && ourExternalId) {
        transaction = await storage.getPaymentTransactionByGatewayId(String(ourExternalId));
        if (transaction) {
          console.log(`✅ CODEXPAY WEBHOOK: Transação encontrada pelo ourExternalId: ${ourExternalId}`);
        }
      }

      if (!transaction) {
        console.warn('⚠️ CODEXPAY WEBHOOK: Transação não encontrada para os IDs:', { gatewayTransactionId, ourExternalId });
        return; // Já respondemos 200
      }

      console.log(`📋 CODEXPAY WEBHOOK: Transação ID=${transaction.id}, Status atual=${transaction.status}, Tipo=${transaction.type}`);

      // Evitar reprocessamento
      if (transaction.status === 'completed' || transaction.status === 'approved') {
        console.log('ℹ️ CODEXPAY WEBHOOK: Transação já processada. Ignorando.');
        return;
      }

      // Verificar status do pagamento
      const statusStr = String(data.status || payload.status || '').toUpperCase();
      const isSuccess = ['PAID', 'COMPLETED', 'COMPLETO', 'APPROVED', 'SUCESSO', 'CONCLUIDO', 'PAGO'].includes(statusStr);
      const isFailure = ['FAILED', 'REJECTED', 'CANCELLED', 'FALHOU', 'REJEITADO', 'CANCELADO'].includes(statusStr);

      console.log(`ℹ️ CODEXPAY WEBHOOK: Status: ${statusStr} → ${isSuccess ? '✅ SUCESSO' : isFailure ? '❌ FALHA' : '⏳ OUTRO'}`);

      if (isSuccess) {
        if (transaction.type === 'deposit') {
          console.log(`💰 CODEXPAY WEBHOOK: Processando DEPÓSITO R$ ${transaction.amount} para usuário ${transaction.userId}`);

          // Atualizar status da transação
          const updatedTx = await storage.updatePaymentTransactionStatus(
            transaction.id,
            'completed',
            String(gatewayTransactionId || transaction.externalId),
            undefined,
            payload
          );
          ...
          if (updatedTx) {
            ...
            const depositAmount = Number(updatedTx.amount);

            // Verificar e aplicar bônus de primeiro depósito
            try {
              const sysSettings = await storage.getSystemSettings();
              if (sysSettings?.firstDepositBonusEnabled) {
                const hasBonus = await storage.hasUserReceivedFirstDepositBonus(userId);
                if (!hasBonus) {
                  let bonusAmount = 0;
                  const pct = sysSettings.firstDepositBonusPercentage || 0;
                  const maxBonus = sysSettings.firstDepositBonusMaxAmount || 0;

                  if (pct > 0) {
                    bonusAmount = (depositAmount * pct) / 100;
                    if (maxBonus > 0 && bonusAmount > maxBonus) bonusAmount = maxBonus;
                  } else {
                    bonusAmount = sysSettings.firstDepositBonusAmount || 0;
                  }

                  bonusAmount = parseFloat(bonusAmount.toFixed(2));

                  if (bonusAmount > 0) {
                    const rolloverAmount = bonusAmount * (sysSettings.firstDepositBonusRollover || 3);
                    const expirationDays = sysSettings.firstDepositBonusExpiration || 7;
                    ...
                    await storage.createUserBonus({
                      ...
                      status: 'active',
                      expiresAt,
                      relatedTransactionId: updatedTx.id
                    });

                    await storage.updateUserBonusBalance(userId, bonusAmount);
                    console.log(`🎁 CODEXPAY WEBHOOK: Bônus de R$ ${bonusAmount} aplicado para usuário ${userId}`);
                  }
                }
              }
            } catch (bonusErr) {
              console.error('❌ CODEXPAY WEBHOOK: Erro ao processar bônus:', bonusErr);
            }

            // Atualizar saldo principal do usuário
            const updatedUser = await storage.updateUserBalance(userId, depositAmount);
            console.log(`✅ CODEXPAY WEBHOOK: Saldo do usuário ${userId} atualizado. Novo saldo: R$ ${updatedUser?.balance}`);
          ...
          console.log(`💸 CODEXPAY WEBHOOK: Processando SAQUE R$ ${transaction.amount} para usuário ${transaction.userId}`);

          await storage.updatePaymentTransactionStatus(transaction.id, 'completed', undefined, undefined, payload);

          // Atualizar status do saque interno
          const responseData = transaction.gatewayResponse as any;
          const internalWithdrawalId = responseData?.internalWithdrawalId || responseData?.withdrawalId;
          ...
          if (internalWithdrawalId) {
            try {
              await storage.updateWithdrawalStatus(
                ...
                undefined,
                undefined,
                'Saque confirmado via webhook CodexPay'
              );
              console.log(`✅ CODEXPAY WEBHOOK: Saque interno ID ${internalWithdrawalId} marcado como aprovado.`);
            } catch (swErr) {
              // Pode já estar aprovado - ignorar
              console.warn('⚠️ CODEXPAY WEBHOOK: Erro ao atualizar saque interno (pode já estar aprovado):', swErr);
            }
          }
        }
      } else if (isFailure) {
        console.log(`❌ CODEXPAY WEBHOOK: Transação ${transaction.id} falhou/foi rejeitada.`);
        await storage.updatePaymentTransactionStatus(transaction.id, 'rejected', undefined, undefined, payload);

        // Para saques falhos: devolver saldo ao usuário
        if (transaction.type === 'withdrawal') {
          await storage.updateUserBalance(transaction.userId, Number(transaction.amount));
          console.log(`💰 CODEXPAY WEBHOOK: Saldo de R$ ${transaction.amount} devolvido ao usuário ${transaction.userId}`);

          const responseData = transaction.gatewayResponse as any;
          const internalWithdrawalId = responseData?.internalWithdrawalId;
          if (internalWithdrawalId) {
            try {
              await storage.updateWithdrawalStatus(
                internalWithdrawalId,
                'rejected',
                undefined,
                `Rejeitado pelo gateway CodexPay: ${statusStr}`,
                'Cancelado via webhook CodexPay'
              );
            } catch (swErr) {
              console.warn('⚠️ CODEXPAY WEBHOOK: Erro ao marcar saque como rejeitado:', swErr);
            }
          }
        }
      } else {
        console.log(`⏳ CODEXPAY WEBHOOK: Status ${statusStr} não requer ação. Aguardando atualização.`);
      }

    } catch (error) {
      console.error('🔥 CODEXPAY WEBHOOK: Erro ao processar:', error);
      // Não rejeitar com 500 pois já respondemos 200
    }
  });
