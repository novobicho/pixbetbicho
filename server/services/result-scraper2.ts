        ...
        try {
            ...
            $('h2, h3, h4, div.title').each((i: number, el: any) => {
                ...
                if (titleText.includes(normalizedDrawName)) {
                    ...
                    if (table) {
                        // Se encontrarmos um match exato, usamos ele.
                        // Se houver múltiplos (ex: nome do sistema é substring de dois títulos),
                        // pegamos o primeiro ou poderíamos refinar.
                        // Mas assumindo nomes completos no sistema, deve ser único.
                        if (!bestMatchTable) {
                            bestMatchTable = table;
                            bestMatchTitle = titleText;
                            // return false; // Parar no primeiro match? Talvez seguro continuar para ver se tem outro melhor?
                            // O usuário disse "nome completo", então deve ser único.
                        }
                    }
                }
            });
