import {
  ...
  type DrawTemplate, type InsertDrawTemplate
} from "@shared/schema";
import express from "express";
import session from "express-session";
import { eq, and, gt, lt, gte, lte, desc, asc, sql, count, inArray } from "drizzle-orm";
import { db, pool } from "./db";
import { drawTemplates } from "@shared/schema";
...
export interface IStorage {
  ...
  updateDrawResult(
    ...
    resultAnimalId6?: number,
    resultAnimalId7?: number,
    resultAnimalId8?: number,
    resultAnimalId9?: number,
    resultAnimalId10?: number,
  ...
  getCompletedDraws(date?: Date): Promise<Draw[]>;

  // Draw Templates Management
  createDrawTemplate(template: InsertDrawTemplate): Promise<DrawTemplate>;
  getDrawTemplates(): Promise<DrawTemplate[]>;
  updateDrawTemplate(id: number, template: Partial<DrawTemplate>): Promise<DrawTemplate | undefined>;
  deleteDrawTemplate(id: number): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  constructor() {
    this.sessionStore = new PostgresSessionStore({
      pool,
      createTableIfMissing: false // Desativado para evitar crash no startup; tabela é criada em initializeData
    });
  ...
  async createDrawTemplate(template: InsertDrawTemplate): Promise<DrawTemplate> {
    const [newTemplate] = await db.insert(drawTemplates).values(template).returning();
    return newTemplate;
  }

  async getDrawTemplates(): Promise<DrawTemplate[]> {
    return await db.select().from(drawTemplates).orderBy(drawTemplates.id);
  }

  async updateDrawTemplate(id: number, template: Partial<DrawTemplate>): Promise<DrawTemplate | undefined> {
    const [updated] = await db.update(drawTemplates).set(template).where(eq(drawTemplates.id, id)).returning();
    return updated;
  }

  async deleteDrawTemplate(id: number): Promise<void> {
  ...
  async createUserBonus(bonus: Partial<UserBonus>): Promise<UserBonus> {
    const [newBonus] = await db.insert(userBonuses).values(bonus as any).returning();
    return newBonus;
  }

  async getUserBonus(bonusId: number): Promise<UserBonus | undefined> {
  ...
  async getActiveBonuses(userId: number): Promise<UserBonus[]> {
    return await db.select().from(userBonuses).where(and(eq(userBonuses.userId, userId), eq(userBonuses.status, "active")));
  }

  async getExpiredBonuses(): Promise<UserBonus[]> {
    return await db.select().from(userBonuses).where(and(eq(userBonuses.status, "active"), lt(userBonuses.expiresAt, new Date())));
  }

  async updateBonusProgress(bonusId: number, newProgress: number): Promise<UserBonus | undefined> {
    const [updatedBonus] = await db.update(userBonuses).set({ rolledAmount: newProgress }).where(eq(userBonuses.id, bonusId)).returning();
    return updatedBonus;
  }

  async completeBonus(bonusId: number): Promise<UserBonus | undefined> {
    const [updatedBonus] = await db.update(userBonuses).set({ status: "completed" }).where(eq(userBonuses.id, bonusId)).returning();
    return updatedBonus;
  }

  async expireBonus(bonusId: number): Promise<UserBonus | undefined> {
  ...
  async getUpcomingDraws(): Promise<Draw[]> {
    ...
    // Subtraímos 3 horas do horário UTC atual para obter o horário BRT equivalente ao armazenado no banco
    ...
    // Início do dia atual (baseado no horário BRT)
    const startOfToday = new Date(brtNow);
    startOfToday.setHours(0, 0, 0, 0);

    // Fim de amanhã (baseado no horário BRT)
    ...
    // Buscar sorteios pendentes de HOJE e AMANHÃ
    // Não filtramos por horário na query para evitar problemas com o formato da data (que pode ser apenas YYYY-MM-DD)
    const potentialDraws = await db
      .select()
      .from(draws)
      .where(
        and(
    ...
    // Filtrar em memória combinando data e hora
    const upcomingDraws = potentialDraws.filter(draw => {
      try {
        // draw.date é um objeto Date (provavelmente 00:00:00Z)
        // draw.time é uma string "HH:MM"
        const [hours, minutes] = draw.time.split(':').map(Number);

        // Criar data do sorteio combinando a data do banco com a hora da string
        const drawDateTime = new Date(draw.date);
        drawDateTime.setHours(hours, minutes, 0, 0);

        // Comparar com o tempo de corte
        // drawDateTime está em "UTC que representa BRT" (porque setamos horas nele)
    ...
    // Adicionar prefixo (Hoje) ou (Amanhã) no nome
    const drawsWithPrefix = upcomingDraws.map(draw => {
      const drawDate = new Date(draw.date);

      // Usar toLocaleDateString para comparação (formato: DD/MM/YYYY)
  ...
  async getCompletedDraws(date?: Date): Promise<Draw[]> {
    const conditions = [eq(draws.status, "completed")];

    if (date) {
      // A data vem do frontend já ajustada para o horário do Brasil
      // Vamos filtrar pelo dia completo em UTC
      ...
      console.log(`[getCompletedDraws] Filtrando resultados:`);
      console.log(`  Data solicitada: ${date.toISOString()}`);
      console.log(`  Início do dia (UTC): ${startOfDay.toISOString()}`);
      console.log(`  Fim do dia (UTC): ${endOfDay.toISOString()}`);

      conditions.push(and(gte(draws.date, startOfDay), lte(draws.date, endOfDay)));
    }

    const completedDraws = await db
      .select()
      .from(draws)
      .where(and(...conditions))
      .orderBy(desc(draws.date), desc(draws.time));

    return completedDraws;
  }

  async getRecentDraws(limit: number = 10): Promise<Draw[]> {
    return await db
      .select()
      .from(draws)
   ...
   */
  async updateBetStatus(betId: number, status: "won" | "lost", winAmount?: number): Promise<void> {
  ...
  async updateUserBalance(userId: number, amount: number): Promise<void> {
    console.log(`Atualizando saldo do usuário ${userId}, adicionando: ${amount}`);
    ...
    console.log(`Saldo do usuário ${userId} atualizado: ${user.balance} → ${newBalance}`);
  }

  /**
   * Busca um game mode por ID
   */
  async getGameMode(gameModeId: number): Promise<GameMode | undefined> {
  ...
  async updateDrawResult(
    ...
    console.log(`Updating draw result for draw ID: ${drawId}, winner animals: 
      ...
      6º prêmio: ${resultAnimalId6 || 'não definido'}, número: ${resultNumber6 || 'não definido'}
      7º prêmio: ${resultAnimalId7 || 'não definido'}, número: ${resultNumber7 || 'não definido'}
      8º prêmio: ${resultAnimalId8 || 'não definido'}, número: ${resultNumber8 || 'não definido'}
      9º prêmio: ${resultAnimalId9 || 'não definido'}, número: ${resultNumber9 || 'não definido'}
      10º prêmio: ${resultAnimalId10 || 'não definido'}, número: ${resultNumber10 || 'não definido'}
    `);
    ...
    console.log(`   1º Prêmio: Animal ID ${resultAnimalId}, Número ${resultNumber1}`);
    ...
    if (drawBets.length === 0) {
      console.log(`⚠️  ATENÇÃO: Nenhuma aposta encontrada para este sorteio!`);
      console.log(`${'='.repeat(60)}\n`);
      return draw;
    }
    ...
    for (const bet of drawBets) {
      ...
      if (premioType === "1-5") {
        ...
        // Apostou em todos os prêmios (1º ao 10º) - dividir o multiplicador por 10
        appliedMultiplier = 0.1; // dividir por 10
        console.log(`Aposta em todos os prêmios (1-10), multiplicador ajustado para ${appliedMultiplier}`);
      }
      ...
      switch (bet.type) {
        case "group": // Grupo (1 animal)
          if ((premioType === "1" && bet.animalId === resultAnimalId) ||
            ...
            (premioType === "6" && bet.animalId === resultAnimalId6) ||
            (premioType === "7" && bet.animalId === resultAnimalId7) ||
            (premioType === "8" && bet.animalId === resultAnimalId8) ||
            (premioType === "9" && bet.animalId === resultAnimalId9) ||
            (premioType === "10" && bet.animalId === resultAnimalId10) ||
            (premioType === "1-5" && (
            ...
            (premioType === "1-10" && (
              ...
              bet.animalId === resultAnimalId4 ||
              bet.animalId === resultAnimalId5 ||
              bet.animalId === resultAnimalId6 ||
              bet.animalId === resultAnimalId7 ||
              bet.animalId === resultAnimalId8 ||
              bet.animalId === resultAnimalId9 ||
              bet.animalId === resultAnimalId10
            ))) {
            isWinner = true;
          }
          break;
        ...
        case "duque_grupo": // Duque de Grupo (2 animais)
          ...
          if (bet.animalId && bet.animalId2) {
            if (premioType === "1" &&
              ...
              // Verificar todos os prêmios 1-10
              let win = false;
              ...
              // Prêmios 6-10
              if (resultAnimalId6 &&
                ((bet.animalId === resultAnimalId6 && bet.animalId2 === resultAnimalId6) ||
                  (bet.animalId2 === resultAnimalId6 && bet.animalId === resultAnimalId6))) {
                win = true;
                console.log(`Duque de Grupo ganhou no 6° prêmio: ${bet.animalId} e ${bet.animalId2}`);
              }

              if (resultAnimalId7 &&
                ((bet.animalId === resultAnimalId7 && bet.animalId2 === resultAnimalId7) ||
                  (bet.animalId2 === resultAnimalId7 && bet.animalId === resultAnimalId7))) {
                win = true;
                console.log(`Duque de Grupo ganhou no 7° prêmio: ${bet.animalId} e ${bet.animalId2}`);
              }

              if (resultAnimalId8 &&
                ((bet.animalId === resultAnimalId8 && bet.animalId2 === resultAnimalId8) ||
                  (bet.animalId2 === resultAnimalId8 && bet.animalId === resultAnimalId8))) {
                win = true;
                console.log(`Duque de Grupo ganhou no 8° prêmio: ${bet.animalId} e ${bet.animalId2}`);
              }

              if (resultAnimalId9 &&
                ((bet.animalId === resultAnimalId9 && bet.animalId2 === resultAnimalId9) ||
                  (bet.animalId2 === resultAnimalId9 && bet.animalId === resultAnimalId9))) {
                win = true;
                console.log(`Duque de Grupo ganhou no 9° prêmio: ${bet.animalId} e ${bet.animalId2}`);
              }

              if (resultAnimalId10 &&
                ((bet.animalId === resultAnimalId10 && bet.animalId2 === resultAnimalId10) ||
                  (bet.animalId2 === resultAnimalId10 && bet.animalId === resultAnimalId10))) {
                win = true;
                console.log(`Duque de Grupo ganhou no 10° prêmio: ${bet.animalId} e ${bet.animalId2}`);
              }
        ...
        case "duque_dezena": // Duque de Dezena (2 dezenas)
          if (bet.betNumbers && bet.betNumbers.length >= 2) {
            ...
            // Processar prêmios usando resultNumberX prioritariamente
            if (resultNumber1) {
        ...
        case "terno_dezena": // Terno de Dezena (3 dezenas)
          if (bet.betNumbers && bet.betNumbers.length >= 3) {
            ...
            const checkTernoDezena = (prize: string) => {
              // Se não tiver resultado para este prêmio, retorna falso
        ...
        case "dozen": // Dezena (2 dígitos)
          if (bet.betNumbers && bet.betNumbers.length > 0) {
            ...
            if (resultNumber6) {
              ...
              const dezena = getDezenaFromMilhar(resultNum);
              console.log(`Resultado 6° prêmio (Milhar): ${resultNum}, dezena: ${dezena}`);
            ...
            if (resultNumber7) {
              ...
              const dezena = getDezenaFromMilhar(resultNum);
              console.log(`Resultado 7° prêmio (Milhar): ${resultNum}, dezena: ${dezena}`);
            ...
            if (resultNumber8) {
              ...
              const dezena = getDezenaFromMilhar(resultNum);
              console.log(`Resultado 8° prêmio (Milhar): ${resultNum}, dezena: ${dezena}`);
            ...
            if (resultNumber9) {
              ...
              const dezena = getDezenaFromMilhar(resultNum);
              console.log(`Resultado 9° prêmio (Milhar): ${resultNum}, dezena: ${dezena}`);
            ...
            // Verificar 10º prêmio
            if (resultNumber10) {
              const resultNum = resultNumber10.padStart(4, '0');
              const dezena = getDezenaFromMilhar(resultNum);
              console.log(`Resultado 10° prêmio (Milhar): ${resultNum}, dezena: ${dezena}`);
            ...
            if (premioType === "1" && prizeResults["1"] === betNumber) {
              ...
              isWinner = true;
              console.log(`Aposta de dezena ${betNumber} ganhou no 6° prêmio`);
            } else if (premioType === "7" && prizeResults["7"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de dezena ${betNumber} ganhou no 7° prêmio`);
            } else if (premioType === "8" && prizeResults["8"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de dezena ${betNumber} ganhou no 8° prêmio`);
            } else if (premioType === "9" && prizeResults["9"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de dezena ${betNumber} ganhou no 9° prêmio`);
            } else if (premioType === "10" && prizeResults["10"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de dezena ${betNumber} ganhou no 10° prêmio`);
            } else if (premioType === "1-5") {
        ...
        case "hundred": // Centena (3 dígitos)
          if (bet.betNumbers && bet.betNumbers.length > 0) {
            ...
            if (resultNumber6) {
              const resultNum = resultNumber6.padStart(4, '0');
              const centena = getCentenaFromMilhar(resultNum);
              console.log(`Resultado 6° prêmio (Milhar): ${resultNum}, centena: ${centena}`);
            ...
            if (resultNumber7) {
              const resultNum = resultNumber7.padStart(4, '0');
              const centena = getCentenaFromMilhar(resultNum);
              console.log(`Resultado 7° prêmio (Milhar): ${resultNum}, centena: ${centena}`);
            ...
            if (resultNumber8) {
              const resultNum = resultNumber8.padStart(4, '0');
              const centena = getCentenaFromMilhar(resultNum);
              console.log(`Resultado 8° prêmio (Milhar): ${resultNum}, centena: ${centena}`);
            ...
            if (resultNumber9) {
              const resultNum = resultNumber9.padStart(4, '0');
              const centena = getCentenaFromMilhar(resultNum);
              console.log(`Resultado 9° prêmio (Milhar): ${resultNum}, centena: ${centena}`);
            ...
            if (resultNumber10) {
              const resultNum = resultNumber10.padStart(4, '0');
              const centena = getCentenaFromMilhar(resultNum);
              console.log(`Resultado 10° prêmio (Milhar): ${resultNum}, centena: ${centena}`);
            ...
            if (premioType === "1" && prizeResults["1"] === betNumber) {
              ...
              isWinner = true;
              console.log(`Aposta de centena ${betNumber} ganhou no 6° prêmio`);
            } else if (premioType === "7" && prizeResults["7"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de centena ${betNumber} ganhou no 7° prêmio`);
            } else if (premioType === "8" && prizeResults["8"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de centena ${betNumber} ganhou no 8° prêmio`);
            } else if (premioType === "9" && prizeResults["9"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de centena ${betNumber} ganhou no 9° prêmio`);
            } else if (premioType === "10" && prizeResults["10"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de centena ${betNumber} ganhou no 10° prêmio`);
            } else if (premioType === "1-5") {
              // Para apostas em todos os prêmios, verificar todos
              const winners = Object.keys(prizeResults).filter(key => prizeResults[key] === betNumber);
              if (winners.length > 0) {
        ...
        case "thousand": // Milhar (4 dígitos)
          if (bet.betNumbers && bet.betNumbers.length > 0) {
            ...
            if (resultNumber6) {
              // Garantir que a milhar do resultado tenha 4 dígitos
              const resultNum = resultNumber6.padStart(4, '0');
              console.log(`Resultado 6° prêmio (Milhar completa): ${resultNum}`);
            ...
            if (resultNumber7) {
              // Garantir que a milhar do resultado tenha 4 dígitos
              const resultNum = resultNumber7.padStart(4, '0');
              console.log(`Resultado 7° prêmio (Milhar completa): ${resultNum}`);
            ...
            if (resultNumber8) {
              // Garantir que a milhar do resultado tenha 4 dígitos
              const resultNum = resultNumber8.padStart(4, '0');
              console.log(`Resultado 8° prêmio (Milhar completa): ${resultNum}`);
            ...
            if (resultNumber9) {
              // Garantir que a milhar do resultado tenha 4 dígitos
              const resultNum = resultNumber9.padStart(4, '0');
              console.log(`Resultado 9° prêmio (Milhar completa): ${resultNum}`);
            ...
            if (resultNumber10) {
              // Garantir que a milhar do resultado tenha 4 dígitos
              const resultNum = resultNumber10.padStart(4, '0');
              console.log(`Resultado 10° prêmio (Milhar completa): ${resultNum}`);
              ...
              if (animal && animal.numbers) {
                for (const numero of animal.numbers) {
                  if (numero === betNumber) {
                    ...
                    console.log(`Número ${betNumber} encontrado no animal do 6° prêmio: ${animal.name}`);
                    break;
                  }
                }
              }
              ...
              if (animal && animal.numbers) {
                for (const numero of animal.numbers) {
                  if (numero === betNumber) {
                    ...
                    console.log(`Número ${betNumber} encontrado no animal do 7° prêmio: ${animal.name}`);
                    break;
                  }
                }
              }
              ...
              if (animal && animal.numbers) {
                for (const numero of animal.numbers) {
                  if (numero === betNumber) {
                    ...
                    console.log(`Número ${betNumber} encontrado no animal do 8° prêmio: ${animal.name}`);
                    break;
                  }
                }
              }
              ...
              if (animal && animal.numbers) {
                for (const numero of animal.numbers) {
                  if (numero === betNumber) {
                    ...
                    console.log(`Número ${betNumber} encontrado no animal do 9° prêmio: ${animal.name}`);
                    break;
                  }
                }
              }
              ...
              if (animal && animal.numbers) {
                for (const numero of animal.numbers) {
                  if (numero === betNumber) {
                    ...
                    console.log(`Número ${betNumber} encontrado no animal do 10° prêmio: ${animal.name}`);
                    break;
                  }
                }
              }
            ...
            if (premioType === "1" && prizeResults["1"] === betNumber) {
              ...
              isWinner = true;
              console.log(`Aposta de milhar ${betNumber} ganhou no 6° prêmio`);
            } else if (premioType === "7" && prizeResults["7"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de milhar ${betNumber} ganhou no 7° prêmio`);
            } else if (premioType === "8" && prizeResults["8"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de milhar ${betNumber} ganhou no 8° prêmio`);
            } else if (premioType === "9" && prizeResults["9"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de milhar ${betNumber} ganhou no 9° prêmio`);
            } else if (premioType === "10" && prizeResults["10"] === betNumber) {
              isWinner = true;
              console.log(`Aposta de milhar ${betNumber} ganhou no 10° prêmio`);
            } else if (premioType === "1-5") {
              // Para apostas em todos os prêmios, verificar todos
              const winners = Object.keys(prizeResults).filter(key => prizeResults[key] === betNumber);
              if (winners.length > 0) {
