...
import { registerUserValidator, loginValidator } from "./validators/user.validators";
...
export function setupAuth(app: Express) {
  ...
  app.post("/api/register", validate(registerUserValidator), async (req, res, next) => {
    try {
      ...
      try {
        ...
        if (systemSettings?.signupBonusEnabled) {
          logger.info(`[Bônus] Detectado bônus de cadastro ativado para o novo usuário ${user.id} (${user.username})`);
          ...
          if (!hasSignupBonus) {
            ...
            if (bonusAmount > 0) {
              ...
              logger.info(`[Bônus] Bônus de cadastro aplicado com sucesso: R$ ${bonusAmount.toFixed(2)}`);
              logger.info(`[Bônus] Detalhes: Rollover R$ ${rolloverAmount.toFixed(2)}, Expira em ${expirationDays} dias`);
            } else {
              logger.warn(`[Bônus] Bônus de cadastro está ativado mas o valor é zero ou inválido: ${bonusAmount}`);
            }
          } else {
            logger.info(`[Bônus] Usuário ${user.id} já recebeu bônus de cadastro anteriormente`);
          }
        } else {
          logger.info('[Bônus] Bônus de cadastro está desativado nas configurações do sistema');
        }
      } catch (bonusError) {
        logger.error("[Bônus] Erro ao aplicar bônus de cadastro:", bonusError);
  ...
  app.post("/api/login", validate(loginValidator), async (req, res, next) => {
    try {
