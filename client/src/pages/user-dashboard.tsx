...
export default function UserDashboard() {
  ...
  // Buscar configurações do sistema (para banners)
  const { data: systemSettings } = useQuery<{ bannerDashboardUrl?: string }>({
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        {/* Header compacto com saudação e saldo */}
        <div className="bg-primary rounded-2xl shadow-xl p-4 sm:p-6 text-white mb-4 relative overflow-hidden">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            {/* Saldo e botões */}
            <div className="bg-white/20 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-3 text-white">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="bg-white/20 hover:bg-white/30 transition-colors rounded-lg px-3 py-1.5 text-sm font-medium flex items-center"
                  >
                  ...
                  <button
                    ...
                    className="bg-green-500/60 hover:bg-green-500/80 transition-colors rounded-lg px-3 py-1.5 text-sm font-medium flex items-center"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Banner promocional com cantos arredondados */}
        <div className="mb-4 rounded-2xl overflow-hidden shadow-lg">
          <div className="relative w-full h-[160px]">
            <img
              src={systemSettings?.bannerDashboardUrl || "/img/banner-dashboard.jpg"}
              alt="Banner promocional"
              className="w-full h-full object-cover"
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                if (!target.src.includes("/img/banner-dashboard.jpg")) {
                  target.src = "/img/banner-dashboard.jpg";
                }
              }}
            />
          </div>
