    ...
    if (bet.status === "pending") {
      ...
      return (
        <Badge variant="outline" className="bg-green-100 text-green-800 hover:bg-green-100">
          Ganhou {formatCurrency(bet.winAmount || 0)}
        </Badge>
      );
    } else {
      return (
              ...
              {sortedBets.map((bet) => (
                <TableRow key={bet.id}>
                  ...
                  <TableCell className="whitespace-nowrap">
                    ...
                    {bet.premioType === "6" && "6° Prêmio"}
                    {bet.premioType === "7" && "7° Prêmio"}
                    {bet.premioType === "8" && "8° Prêmio"}
                    {bet.premioType === "9" && "9° Prêmio"}
                    {bet.premioType === "10" && "10° Prêmio"}
                    {bet.premioType === "1-5" && "1° ao 5° Prêmio"}
                    {bet.premioType === "1-10" && "1° ao 10° Prêmio"}
                  </TableCell>
                  <TableCell>{formatCurrency(bet.amount)}</TableCell>
                  <TableCell className="text-emerald-600 font-medium">
                    {bet.potentialWinAmount ? formatCurrency(bet.potentialWinAmount) : "-"}
                    {bet.premioType === "1-5" && <span className="text-xs text-gray-500 ml-1">(por prêmio)</span>}
                  </TableCell>
        ...
        <div className="md:hidden space-y-4">
          {sortedBets.map((bet) => (
            ...
            >
              ...
              <div className={`flex justify-between items-center p-3 border-b ${bet.status === 'won'
                ...
                <div className="font-medium text-gray-800 flex items-center">
                  <div className={`w-2 h-2 rounded-full mr-2 ${bet.status === 'won'
                      ? 'bg-green-500'
                      : bet.status === 'pending'
                        ? 'bg-yellow-500'
                        : 'bg-red-500'
                ...
                <div className="grid grid-cols-2 gap-3 mb-2">
                  ...
                  <div className="bg-gray-50 rounded-lg p-2 text-center">
                    ...
                    <div className="text-sm font-semibold text-emerald-600">
                      {bet.potentialWinAmount
                        ? formatCurrency(bet.potentialWinAmount)
                        : "-"
                      }
                      {bet.premioType === "1-5" && (
                        <div className="text-xs text-gray-500">(por prêmio)</div>
                      )}
                    </div>
                  </div>
                </div>
